import torch
from torch.utils.data import DataLoader
import imageio
import numpy as np
import image
import cv2
from PIL import Image
from torchvision import transforms
from torch.utils.data.dataset import Dataset
import torch.utils.data.dataset


#_npy_paths = np.load('./save_npy/all_train_video_check.npy')
#all_train_gts = np.load('./save_npy/all_train_gts.npy')
#all_img_train_forehead = np.load('./save_npy/all_train_forehead.npy')
#all_img_train_check = np.load('./save_npy/all_train_check.npy')
#print(len(all_img_train_forehead[0]))
device = 'cuda' if torch.cuda.is_available() else 'cpu'


class customDataset(Dataset):
    def __init__(self, root_forehead, root_gts, split, transform):
        #
        # initial paths, transform,
        #
        self.transform = transform
        device = 'cuda' if torch.cuda.is_available() else 'cpu'

        # load img and gt path
        self.foreheads = np.load(root_forehead)
        #self.check = np.load(root_check)
        self.gts = np.load(root_gts)
        print(len(self.foreheads))
        print(len(self.gts))

        #assert len(self.foreheads) == len(self.gts), 'mismatched length!!'
        print('Total data in {} split: {}'.format(split, len(self.foreheads)))

    def __getitem__(self, index):
        # read file preprocess np -> tensor return forehead and check  30->1
        # has got index
        # first read first tensor in image 1

        for i, path in enumerate(self.foreheads[index]):  # from paths get every 30 path
            forehead_img = Image.open(self.foreheads[index][i])
            #check_img = Image.open(self.check[index][i])
            if self.transform is not None:
                forehead_img = self.transform(forehead_img)
                forehead_img = forehead_img.cuda()
                forehead_img = torch.unsqueeze(forehead_img, 1)
                #check_img = self.transform(check_img)
                #check_img = check_img.cuda()
                #check_img = torch.unsqueeze(check_img, 1)
            if i == 0:
                forehead = forehead_img
                #check = check_img
            else:

                forehead = torch.cat([forehead, forehead_img], 1)
                #check = torch.cat([check, check_img], 1)

        gt = self.gts[index]

        return forehead, gt

    def __len__(self):
        # return len of imgs
        return len(self.foreheads)


transform = transforms.Compose([
    transforms.ToTensor(),])









